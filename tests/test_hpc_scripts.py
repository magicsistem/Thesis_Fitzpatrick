...
class HPCScriptTests(unittest.TestCase):
    ...
    def test_arrays_are_single_gpu_and_mapping_is_explicit(self):
        ...
        self.assertIn('python - "$1" "$2" "${3}/oof_manifest.json"', b2)
        self.assertIn('--p0-root "$3"', b2)
        self.assertIn("' _ \"$TRAIN_MANIFEST\" \"$FOLDS_FILE\" \"$P0_ROOT\"", b2)
        self.assertNotIn('$P0_ROOT/oof_manifest.json" "$python_bin"', b2)
    ...
    def test_p0_slurm_uses_16_processes_reuse_cache_and_one_thread_libraries(self):
        ...
        self.assertIn("P0_WORKERS=${P0_WORKERS:-16}", p0)
