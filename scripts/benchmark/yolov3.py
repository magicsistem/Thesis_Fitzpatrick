...
from thesis_fitzpatrick.yolo import collect_raw_validation_detections, freeze_detector, patch_yolov3_cfg, prepare_darknet_fold, run_darknet_training, select_validation_configuration  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    train.add_argument("--resume", type=Path); train.add_argument("--output", required=True, type=Path); train.add_argument("--confirm-training", action="store_true")
    ...
    freeze.add_argument("--nms", required=True, type=float); freeze.add_argument("--margin", required=True, type=float); freeze.add_argument("--output", required=True, type=Path)
...
def main() -> None:
    ...
    if args.action == "prepare-fold":
        ...
        print(json.dumps(run_darknet_training(args.darknet, args.data, args.cfg, args.initial_weights, args.output, resume_weights=args.resume), indent=2))
        ...
        print(json.dumps(freeze_detector(args.cfg, args.weights, {"confidence_threshold": args.confidence, "nms_threshold": args.nms, "margin_fraction": args.margin}, args.output, args.validation_report), indent=2))
