...
function outcomeLabel(item) {
  if (item?.failure_is_fatal) return `fallo técnico${item.failure_code ? ` (${item.failure_code})` : ""}`;
  if (item?.outcome === "degenerate_prediction") return `predicción degenerada${item.failure_code ? ` (${item.failure_code})` : ""}`;
...
function renderRun(payload, target) {
  ...
  const renderRanking = () => {
    ...
    if (filtered) rows = methods.map((method) => {
      ...
      return {method_id:method, n:subset.length, technical_failures:subset.filter((item) => item.failure_is_fatal).length, degenerate_predictions:subset.filter((item) => item.outcome === "degenerate_prediction").length, [`${metric}_mean`]:values.length ? values.reduce((sum, value) => sum + value, 0) / values.length : NaN, [`${metric}_median`]:middle};
    });
    target.querySelector(".run-ranking").innerHTML = rows.map((row) => `<tr><td>${escapeHtml(row.method_id)}</td><td>${row.n}</td><td>${row.technical_failures ?? row.failures ?? 0}</td><td>${row.degenerate_predictions ?? 0}</td><td>${Number(row[`${metric}_mean`] ?? NaN).toFixed(4)}</td><td>${Number(row[`${metric}_median`] ?? NaN).toFixed(4)}</td><td>${filtered ? "ND en filtro interactivo" : `${Number(row[`${metric}_ci95_low`] ?? NaN).toFixed(4)}–${Number(row[`${metric}_ci95_high`] ?? NaN).toFixed(4)}`}</td></tr>`).join("") || `<tr><td colspan="7">Sin métricas autorizadas para esta condición.</td></tr>`;
  };
  const render = () => {
    ...
    target.querySelector(".run-result").innerHTML = `<h3>${escapeHtml(item.condition ?? item.evaluation)} · ${escapeHtml(item.method_id)} · ${escapeHtml(item.image_id)}</h3><div class="artifact-grid">${cards}</div><p class="runtime">Backend: ${Number(item.backend?.backend_time_ms ?? 0).toFixed(1)} ms · End-to-end: ${Number(item.end_to_end_time_ms ?? 0).toFixed(1)} ms · Parámetros: ${Number(resources.parameter_count ?? 0).toLocaleString()} · Pesos: ${(Number(resources.checkpoint_bytes ?? 0) / 1048576).toFixed(1)} MiB · RAM pico: ${Number(item.backend?.details?.peak_ram_mb ?? 0).toFixed(1)} MiB · Estado: ${escapeHtml(outcomeLabel(item))} · Fallback: ${item.p0_fallback_used ? "sí" : "no"}</p><p class="stats">${metrics}</p>`;
    renderRanking();
  };
